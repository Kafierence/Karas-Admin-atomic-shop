...
const useProductSubmit = (id) => {
  ...
  const handleQuantityPrice = (value, name, id, variant) => {
    ...
    if (name === "price") {
      const discount = value > 0 ? variant.originalPrice - variant.price : 0;
      variants[id]["price"] = value;
      variants[id]["discount"] = Number(variant.originalPrice - variant.price);

      console.log("discount", discount, variant.originalPrice - variant.price);
      setIsBulkUpdate(true);
      const timeOutId = setTimeout(() => setIsBulkUpdate(false), 500);
      return () => clearTimeout(timeOutId);
    }
    ...
    if (name === "quantity") {
      variants[id][name] = Number(parseFloat(value).toFixed(0));
